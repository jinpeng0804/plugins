...
static tuple<bool, bool> JudgeExistAndEmpty(const string &path)
{
    ...
    if (ret < 0) {
        HILOGE("Failed to read directory '%s': %s", path.c_str(), uv_strerror(ret));
        return { true, false };
    }
...
static int RmDirectory(const string &path)
{
    ...
    if (ret < 0) {
        ...
        HILOGE("Failed to stat directory '%s': %s", path.c_str(), uv_strerror(ret));
        return ret;
    }
    ...
    while (uv_fs_scandir_next(&readdirReq, &dent) != UV_EOF) {
        ...
        if (dent.type == UV_DIRENT_DIR) {
            ...
            if (ret < 0) {
                HILOGE("Failed to unlink file '%s': %s", entryPath.c_str(), uv_strerror(ret));
                uv_fs_req_cleanup(&readdirReq);
                return ret;
            }
        }
    ...
    if (ret < 0) {
        HILOGE("Failed to remove directory '%s': %s", path.c_str(), uv_strerror(ret));
        return ret;
    }
    return ERRNO_NOERR;
}
static int RemovePath(const string& pathStr)
{
    ...
    if (ret < 0) {
        ...
        HILOGE("Failed to stat path: %s", uv_strerror(ret));
        return ret;
    }
    if (statReq.statbuf.st_mode & S_IFDIR) {
        ...
        uv_fs_req_cleanup(&unlinkReq);
        if (ret < 0) {
            HILOGE("Failed to unlink file: %s", uv_strerror(ret));
            return ret;
        }
    }
    return ERRNO_NOERR;
...
static int RenameFile(const string &src, const string &dest, const int mode, deque<struct ErrFiles> &errfiles)
{
    ...
    if (ret < 0) {
        HILOGE("Failed to rename file: %s", uv_strerror(ret));
        return ret;
    }
    return ERRNO_NOERR;
}
...
static int RenameDir(const string &src, const string &dest, const int mode, deque<struct ErrFiles> &errfiles)
{
    ...
    if (ret == 0) {
        ...
        HILOGE("Failed to stat destination path: %s", uv_strerror(ret));
        return ret;
    }
    ...
    if (ret == UV_EXDEV) {
        HILOGD("Failed to rename file due to EXDEV");
        uv_fs_t mkdirReq;
        ret = uv_fs_mkdir(nullptr, &mkdirReq, dest.c_str(), 0755, nullptr);
        uv_fs_req_cleanup(&mkdirReq);
        if (ret < 0) {
            HILOGE("Failed to create directory: %s", uv_strerror(ret));
            return ret;
        }
        return RecurMoveDir(src, dest, mode, errfiles);
    } else if (ret < 0) {
        HILOGE("Failed to rename directory: %s", uv_strerror(ret));
        return ret;
    }
    return ERRNO_NOERR;
}
    ...
    deque<struct ErrFiles> &errfiles)
{
    ...
    if (ret < 0) {
        HILOGE("Failed to read directory '%s': %s", srcPath.c_str(), uv_strerror(ret));
        return errno;
    }
